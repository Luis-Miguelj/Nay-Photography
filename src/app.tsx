...
export function App(){
  ...
  return(
    <div>
      <motion.div className="flex flex-col gap-2 w-full bg-zinc-800 justify-center h-screen home-start duration-500" transition={{duration: 1}} animate={{alignItems: 'center'}} id="image-home">
        <motion.h1 className="text-6xl font-semibold text-zinc-100 max-sm:text-3xl" transition={{duration: 1}} animate={{x: [-700,0], opacity: [0,1]}}>Seja bem vindo(a)</motion.h1>
        <motion.p className="text-base font-medium text-zinc-100 max-sm:text-sm" transition={{duration: 1}} animate={{x: [-700,0], opacity: [0,1]}}>Nay Photography</motion.p>
      </motion.div>
            ...
            return(
              <div key={index} className="w-full mx-auto flex flex-col my-10 justify-center items-center" id={`animated-${index}`}>
                <div className="w-3/4 flex flex-col space-y-5 my-5">
                  ...
                  <div className="w-full h-96 max-sm:h-72 rounded shadow" style={
                    {
